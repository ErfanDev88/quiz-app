...
const score = computed(() => {
  let value = 0
  questions.value.map(p => {
    if (p.selected == p.answer) {
      value++
    }
  })
  return value
...
<template>
  <main class="app">
    ...
    <section class="quiz" v-if="!quizCompleted">
      ...
      <div class="option">
        <label v-bind:key="index" v-for="(option, index) in getCurrentQuestion.options" :for="'option' + index" :class="`option ${getCurrentQuestion.selected == index
